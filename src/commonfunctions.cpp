...
void publishPath(int &argc, char **argv, const nav_msgs::Path &trajectory, const nav_msgs::Path &waypoints, std::string &name)
{
	...
	// 1. Check to see if a node with this name is already running
	std::vector<std::string> list;
	ros::master::getNodes(list);
	
	for(int i = 0; i < list.size(); i++)
	{
		std::cout << list[i] << std::endl;

	/*	if(list[i].compare(name + "_visualizer"))
		{
			throw "A trajectory object with this name already exists!";
			ROS_ERROR("A trajectory object with this name already exists!");
		}

	}

	// 2. If not, go ahead and create the node
	ros::init(argc, argv, name + "_visualizer");
	ros::NodeHandle n;


	ros::Publisher pub1 = n.advertise<nav_msgs::Path>(name+"_trajectory",1);	
	ros::Publisher pub2 = n.advertise<nav_msgs::Path>(name+"_waypoints",1);
